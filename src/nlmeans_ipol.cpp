...
int main(int argc, char **argv) {
    if (argc < 5) { // Vérifie le nombre de paramétres en entrée
    ...
    size_t nx, ny, nc;
    float *d_v = NULL;
    d_v = io_png_read_f32(argv[1], &nx, &ny, &nc); // fonction de io_png.cpp
    if (!d_v) {
    ...
    // tester si l'image est vraiment une image couleur même si elle a plus d'un canal
    if (d_c > 1) {
        // d_c égale 3
        int i = 0;
