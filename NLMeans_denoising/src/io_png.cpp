...
static void *io_png_read_raw(const char *fname, size_t * nxp, size_t * nyp, size_t * ncp, int png_transform, int dtype) {
    ...
    if ((PNG_SIG_LEN != fread(png_sig, 1, PNG_SIG_LEN, fp)) || 0 != png_sig_cmp(png_sig, (png_size_t) 0, PNG_SIG_LEN)) {
    ...
    *ncp = (size_t) png_get_channels(png_ptr, info_ptr);
    ...
    switch (dtype) {
        case IO_PNG_U8:
            if (NULL == (data_u8 = (unsigned char *) malloc(size * sizeof(unsigned char)))) {
                return io_png_read_abort(fp, &png_ptr, &info_ptr);
            }
            data = (void *) data_u8;
            for (k = 0; k < *ncp; k++) {
...
unsigned char *io_png_read_u8(const char *fname, size_t * nxp, size_t * nyp, size_t * ncp) {
    ...
    return (unsigned char *) io_png_read_raw(fname, nxp, nyp, ncp, PNG_TRANSFORM_IDENTITY, IO_PNG_U8);
}
...
unsigned char *io_png_read_u8_rgb(const char *fname, size_t * nxp, size_t * nyp) {
    ...
    unsigned char *img;
    /* read the image */
    img = (unsigned char *) io_png_read_raw(fname, nxp, nyp, &nc, PNG_TRANSFORM_STRIP_ALPHA, IO_PNG_U8);
    if (NULL == img) {
        /* error */
        return NULL;
    }
    if (3 == nc) {
        ...
        /* resize the image */
        size = *nxp * *nyp;
        img = (unsigned char *)realloc(img, 3 * size * sizeof(unsigned char));
        img_r = img;
        img_g = img + size;
        img_b = img + 2 * size;
...
unsigned char *io_png_read_u8_gray(const char *fname, size_t * nxp, size_t * nyp) {
    ...
    if (1 == nc) {
        ...
        img_b = img + 2 * size;
        for (i = 0; i < size; i++) {
            img[i] = (unsigned char) ((6969 * img_r[i] + 23434 * img_g[i] + 2365 * img_b[i]) / 32768);
        }
...
float *io_png_read_f32(const char *fname, size_t * nxp, size_t * nyp, size_t * ncp) {
    ...
    return (float *) io_png_read_raw(fname, nxp, nyp, ncp, PNG_TRANSFORM_IDENTITY, IO_PNG_F32);
}
...
float *io_png_read_f32_rgb(const char *fname, size_t * nxp, size_t * nyp) {
    ...
    float *img;
    /* read the image */
    img = (float *) io_png_read_raw(fname, nxp, nyp, &nc, PNG_TRANSFORM_STRIP_ALPHA, IO_PNG_F32);
    if (NULL == img) {
        /* error */
        return NULL;
    }
...
float *io_png_read_f32_gray(const char *fname, size_t * nxp, size_t * nyp) {
    ...
    if (1 == nc) {
        ...
        img_b = img + 2 * size;
        for (i = 0; i < size; i++) {
            img[i] = (float) (0.212671 * img_r[i] + 0.715160 * img_g[i] + 0.072169 * img_b[i]);
        }
        /* resize and return the image */
...
static int io_png_write_raw(const char *fname, const void *data, size_t nx, size_t ny, size_t nc, int dtype) {
    ...
    if (0 != setjmp(png_jmpbuf(png_ptr))) {
        /* if we get here, we had a problem reading the file */
        return io_png_write_abort(fp, idata, row_pointers, &png_ptr, &info_ptr);
    }
    ...
    /* set image header */
    png_set_IHDR(png_ptr, info_ptr, (png_uint_32) nx, (png_uint_32) ny, bit_depth, color_type, interlace, compression, filter);
            ...
            for (k = 0; k < nc; k++) {
                ...
                for (j = 0; j < ny; j++) {
                    ...
                    for (i = 0; i < nx; i++) {
                        ...
                        *idata_ptr = (png_byte) (tmp < 0. ? 0. : (tmp > 255. ? 255. : tmp));
                        idata_ptr += nc;
                    }
                }
            }
...
int io_png_write_u8(const char *fname, const unsigned char *data, size_t nx, size_t ny, size_t nc) {
    return io_png_write_raw(fname, (void *) data, (png_uint_32) nx, (png_uint_32) ny, (png_byte) nc, IO_PNG_U8);
}
...
int io_png_write_f32(const char *fname, const float *data, size_t nx, size_t ny, size_t nc) {
    return io_png_write_raw(fname, (void *) data, (png_uint_32) nx, (png_uint_32) ny, (png_byte) nc, IO_PNG_F32);
}
